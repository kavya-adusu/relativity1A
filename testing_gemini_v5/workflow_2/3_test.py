...
genai.configure(api_key='AIzaSyDh3tBNV4fuZwETUnOK25PN8pf1XDvlt-4')
model = genai.GenerativeModel('gemini-pro')

def test_candidate_responses(set_name, travel_answer, project_answer, relocation_answer):
    prompt = (
        f'You will be evaluating a candidate in the hiring process for a Sales Engineering Role. For context, there will be a question and then an answer from a candidate. This evaluation is for test set "{set_name}". '
...
def evaluate_candidate(test_set):
    try:
        set_name = test_set["set"]
        travel_answer = test_set["travel"]["answer"]
        project_answer = test_set["project"]["answer"]
        relocation_answer = test_set["relocation"]["answer"]
        result = test_candidate_responses(set_name, travel_answer, project_answer, relocation_answer)
        if result:
            return {
                "set": result.get("set", set_name),
...
if __name__ == "__main__":
    input_file = "testing_gemini_v5/workflow_2/all_test_sets_with_labels_cleaned.json"
